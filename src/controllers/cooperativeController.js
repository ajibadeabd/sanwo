...
const update = async (req, res) => {
  try {
    ...
    if (
      req.body.password &&
      !bcrypt.compareSync(req.body.old_password, userExist.password)
    ) {
      return res.status(400).send({
        success: false,
...
const cooperativePaymentReminder = async (req, res) => {
  ...
  notificationEvents.emit("cooperative_member_payment_reminder", {
    user,
    amount
  });
  res.send({
...
const defaultingMembers = async (req, res) => {
  try {
    ...
    const corpertiveMembers = await req.Models.User.find({
      cooperative: req.authData.userId
    })
      .skip(offset)
      .limit(limit);
    ...
    for (const mem of members) {
      ...
      const orders = await req.Models.Order.find({
        buyer: mem._id,
...
const updateMemberStatus = async (req, res) => {
  ...
  notificationEvents.emit("cooperative_member_status_updated", {
    user,
    status
  });
};
