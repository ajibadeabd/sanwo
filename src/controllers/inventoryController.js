...
const create = (req, res) => {
  ...
  if (req.body.installmentPercentagePerMonth) {
    ...
    installmentPercentagePerMonth = JSON.parse(
      req.body.installmentPercentagePerMonth
    );
    installmentPercentagePerMonth = installmentPercentagePerMonth.map(
      percentagePerMonth => parseInt(percentagePerMonth, 10)
    );
  }
  req.Models.Inventory.create(
    ...
    (err, product) => {
      if (err) {
        ...
        notificationEvents.emit("inventory_created", {
          product,
          sellerId: req.authData.userId
        });
      }
    }
  );
...
const getInventories = async (req, res) => {
  try {
    const { model, offset, limit, resultCount } = await _queryInventory(
      req,
      true,
      false
    );
...
const deleteInventory = async (req, res) => {
  try {
    ...
    if (!result) {
      return res
        .status(400)
        .send({ success: false, message: "Inventory Not Found", data: null });
    }
  ...
  req.Models.Inventory.findOne(
    ...
    (err, inventory) => {
      ...
      if (inventory) {
        ...
        const checkImage = productImages.filter(image =>
        ...
        inventory.images = productImages.filter(
...
const searchInventories = async (req, res) => {
  ...
  const products = await req.Models.Inventory.find({
...
const getInventoryStat = async (req, res) => {
  ...
  const totalProducts = await req.Models.Inventory.countDocuments(bySeller);

  const productsOutOfStock = await req.Models.Inventory.countDocuments({
    ...bySeller,
    quantity: { $lte: 0 }
  });

  const productsInStock = await req.Models.Inventory.countDocuments({
    ...bySeller,
    quantity: { $gte: 1 }
  });

  const productsUnapproved = await req.Models.Inventory.countDocuments({
    ...bySeller,
    status: false
  });

  const products = await req.Models.Inventory.find({ ...bySeller });
  let soldItems = 0;
...
const getInventoryInStock = async (req, res) => {
  ...
  const { offset, limit, resultCount } = await _queryInventory(req, true, true);
