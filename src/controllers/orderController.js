...
const createOrdersWithoutInstallment = async (
  req,
  address,
  cartItemsWithoutInstallment
) => {
  ...
  for (let i = 0; i <= cartItemsWithoutInstallment.length; i += 1) {
    if (cartItemsWithoutInstallment[i]) {
      ...
      if (cartItemsWithoutInstallment[i].product.quantity) {
        ...
        cartItemsWithoutInstallment[i].product.quantity -=
          cartItemsWithoutInstallment[i].quantity;
  ...
  return order
    ...
    : Promise.reject(new Error("Order Could't not be created"));
};
const create = async (req, res) => {
  ...
  const cartItemsWithoutInstallment = currentUserCart.filter(
    cartItem => !cartItem.installmentPeriod || cartItem.installmentPeriod === 0
  );

  // Assert that the user has items in cart
  if (!cartItemsWithoutInstallment.length) {
    responsePayload.data.errors.cart = [
      "You don't have any item in your cart, valid for checkout"
    ];
    return res.status(400).send(responsePayload);
  }

  /** Get the user's shipping address */
  const address = await req.Models.AddressBook.findOne({
  ...
  res.send({
    ...
    data: ordersWithoutInstallment
  });
  // Empty current user cart with no installment
  await req.Models.Cart.deleteMany({
...
const _createInstallmentRelatedRecords = async (
  req,
  address,
  cart,
  bankAccount
...
const createInstallmentOrder = async (req, res) => {
  try {
    ...
    const cart = await req.Models.Cart.findOne({
      _id: req.body.cartId,
    ...
    if (!cart) {
      responsePayload.data.errors.cart = [
        "The specified cart item is not available on your list"
      ];
      return res.status(400).send(responsePayload);
    }
    ...
    if (!cart.approvalRecord) {
      responsePayload.data.errors.bankAccountId = [
        "Approval has not been requested for this cart."
      ];
      return res.status(400).send(responsePayload);
    }
    ...
    if (
      cart.approvalRecord.adminApprovalStatus !==
      constants.ORDER_STATUS.approved
    ) {
      responsePayload.data.errors.bankAccountId = [
        "This cart is not valid for checkout, still waiting admin approval"
      ];
      return res.status(400).send(responsePayload);
    }
    // assert that this cart has been approved
    ...
    const bankAccountDetails = await req.Models.BankAccount.findOne({
    ...
    if (!bankAccountDetails) {
      responsePayload.data.errors.bankAccountId = [
        "Invalid bank account record supplied"
      ];
      return res.status(400).send(responsePayload);
    }
    ...
    const orderRecord = await _createInstallmentRelatedRecords(
      req,
      address,
      cart,
      bankAccountDetails.toObject()
    );

    const order = await req.Models.Order.findById(orderRecord._id).populate(
      "buyer purchases"
    );
    ...
    if (
      ...
      return res
        .status(500)
        .send(
          "Oops! An error occurred connecting to Remita. If error persist please notify admin"
        );
    }
