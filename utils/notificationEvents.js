...
class CoreEvents extends EventEmitter {
  ...
  constructor() {
    ...
    this.on(
      "installment_order_approval_mail",
      this.onInstallmentOrderApprovalMail
    );
    ...
    this.on(
      ...
      this.onInstallmentPaymentCompleted
    );
    ...
    this.on(
      "cooperative_member_status_updated",
      this.onCooperativeMemberStatusUpdated
    );
    this.on("send_password_reset_email", this.onSendPasswordResetEmail);
    this.on(
      "installment_cart_approval_mail",
      this.onInstallmentCartApprovalMail
    );
    this.on(
      "seller_installment_cart_declined",
      this.onSellerInstallmentCartDeclined
    );
    this.on(
      "installment_cart_approval_request_admin",
      this.onInstallmentCartApprovalRequestAdmin
    );
    this.on(
      "installment_cart_admin_approved",
      this.onInstallmentCartAdminApproved
    );
    this.on(
      "installment_cart_admin_declined",
      this.onInstallmentCartAdminDeclined
    );
  ...
  sendEmail(template, destination, locals) {
    ...
    const subject_path = path.join(
      "mail_templates/" + template + "/subject.pug"
    );
    ...
    this.sendmail({
      destination,
      content: { html: htmlContent, subject: subjectContent }
    });
  ...
  async onPaymentStatusChanged({ order, buyer, sellerEmails }, status) {
    // Notify the buyer
    this.sendEmail(
      "buyer_payment_status_mail",
      { to: buyer.email },
      { status, buyer, order }
    );
    let adminEmails = await models.User.find({
      accountType: helper.constants.SUPER_ADMIN
    }).select("email -_id");

    // Notify all admin
    adminEmails = adminEmails.map(admin => admin.email);
    this.sendEmail(
      "admin_payment_status_mail",
      { to: `${CoreEvents.convertEmailsToString(adminEmails)}` },
      {
        status,
        buyer,
        adminEmails,
  ...
  async onPaymentReminder({ user, amount }) {
    this.sendEmail(
      "corporative_defaulting_member_reminder",
      { to: user.email },
      { user, amount }
    );
  }
  ...
  async onWalletStatusChanged({ wallet }, status) {
    ...
    this.sendEmail(
      "seller_wallet_status_mail",
      { to: seller.email },
      { seller, purchase, status }
    );
  ...
  async onPurchaseStatusChanged({ order, status, purchase }) {
    this.sendEmail(
      "buyer_purchase_status_mail",
      { to: order.buyer.email },
      { order, status, purchase }
    );
    this.sendEmail(
      "seller_purchase_status_mail",
  ...
  async onNewOrderMail(order) {
    const buyer = await models.User.findOne({ _id: order.buyer }).populate(
      "cooperative"
    );
    const { cart } = order;

    this.sendEmail(
      "buyer_new_order_mail",
      { to: buyer.email },
      { cart, order, buyer }
    );
  ...
  async onInstallmentCartApprovalMail({ cart, approvalRecord }) {
    const adminEmails = await models.User.find({
      accountType: "super_admin"
    }).select("-_id email");
    ...
    this.sendEmail(
      "seller_cart_item_confirmation_mail",
      { to: seller.email },
      {
        cart,
        seller,
    ...
    if (adminEmails.length) {
      this.sendEmail(
        "admin_cart_item_confirmation_mail",
        {
          to: `${CoreEvents.convertEmailsToString(adminEmails)}, ${
            cooperative.email
          }`
  ...
  async onInstallmentPaymentDebit({ order, payment }) {
    ...
    this.sendEmail(
      "buyer_installment_debit_mail",
      { to: order.buyer.email },
      { order, purchase, payment }
    );
    this.sendEmail(
      "seller_installment_debit_mail",
      { to: purchase.seller.email },
      { order, purchase, payment }
    );
    ...
    this.sendEmail(
      "admin_installment_debit_mail",
      { to: `${CoreEvents.convertEmailsToString(adminEmails)}` },
      { order, purchase, payment }
    );
    const { cooperative } = order.buyer;
    this.sendEmail(
      "cooperate_admin_installment_debit_mail",
  ...
  async onInstallmentPaymentCompleted({ order, payment }) {
    const purchase = order.purchases[0];
    this.sendEmail(
      "buyer_installment_completed_mail",
      { to: order.buyer.email },
      { order, purchase, payment }
    );
    this.sendEmail(
      "seller_installment_completed_mail",
      { to: purchase.seller.email },
      { order, purchase, payment }
    );
    ...
    this.sendEmail(
      "admin_installment_completed_mail",
      { to: `${CoreEvents.convertEmailsToString(adminEmails)}` },
      { order, purchase, payment }
    );
    const { cooperative } = order.buyer;
    this.sendEmail(
      "cooperate_admin_installment_completed_mail",
      { to: cooperative.email },
      { order, purchase, payment }
    );
  }
  ...
  async onRegisteredNewBuyer({ user }) {
    const userRecord = await models.User.findById(user._id).populate(
      "cooperative"
    );
  ...
  async onSellerInstallmentCartDeclined(approvalRecord) {
    ...
    this.sendEmail(
      "buyer_installment_cart_declined_mail",
      {
        to: `${CoreEvents.convertEmailsToString(adminEmails)}, ${
          cooperative.email
        }`
    ...
    this.sendEmail(
      "admin_installment_cart_declined_mail",
      {
        to: `${CoreEvents.convertEmailsToString(adminEmails)}, ${
          cooperative.email
        }`
    ...
    if (adminEmails.length) {
      for (let index = 0; index < adminEmails.length; index += 1) {
        if (adminEmails[index].email) {
          this.sendEmail(
            "admin_cart_item_confirmation_request_mail",
  ...
  async onInstallmentCartAdminDeclined(approvalRecord) {
    ...
    // Notify seller
    this.sendEmail(
      "seller_installment_cart_declined_mail",
      { to: seller.email },
      { product, buyer, seller }
    );
